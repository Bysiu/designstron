...
export default function HostingSuccessPage() {
  ...
  const fetchOrderDetails = async () => {
    try {
      const response = await fetch(`/api/hosting/verify-payment?session_id=${session_id}`);
      if (response.ok) {
  ...
  return (
    <>
      <Head>
        <title>Płatność zakończona sukcesem! - DesignStron</title>
      </Head>
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="max-w-2xl mx-auto">
            <div className={`${cardBg} backdrop-blur-sm rounded-2xl border p-8 text-center`}>
              ...
              <h1 className={`text-3xl font-black mb-4 ${textPrimary}`}>Płatność zakończona sukcesem!</h1>
              <p className={`${textSecondary} mb-8`}>
                Dziękujemy za płatność! Twoje zamówienie zostało pomyślnie zrealizowane.
              </p>
              ...
              <div className={`mt-8 p-4 rounded-lg ${isDark ? 'bg-blue-500/10 border-blue-500/20' : 'bg-blue-50 border-blue-200'} border`}>
                <p className={`text-sm ${isDark ? 'text-blue-300' : 'text-blue-700'}`}>
                  <strong>Ważne:</strong> Potwierdzenie płatności zostało wysłane na Twój adres e-mail.
                </p>
              </div>
            </div>
          </div>
