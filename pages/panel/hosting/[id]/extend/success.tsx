...
export default function ExtendHostingSuccessPage() {
  ...
  const fetchOrderDetails = async () => {
    try {
      // Weryfikuj płatność Stripe
      const response = await fetch(`/api/stripe/verify-payment?session_id=${session_id}`);
      if (response.ok) {
        const data = await response.json();
        console.log('Płatność przedłużenia hostingu zweryfikowana:', data);
        ...
        console.error('Błąd weryfikacji płatności');
        // Przekieruj do panelu w przypadku błędu
  ...
  return (
    <>
      <Head>
        <title>Przedłużenie hostingu - Sukces! - DesignStron</title>
      </Head>
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="max-w-2xl mx-auto">
            <div className={`${cardBg} backdrop-blur-sm rounded-2xl border p-8 text-center`}>
              ...
              <h1 className={`text-3xl font-black mb-4 ${textPrimary}`}>Przedłużenie hostingu zakończone sukcesem!</h1>
              <p className={`${textSecondary} mb-8`}>
                Twój hosting został pomyślnie przedłużony. Dziękujemy za zaufanie!
              </p>
              ...
              <div className={`mt-8 p-4 rounded-lg ${isDark ? 'bg-blue-500/10 border-blue-500/20' : 'bg-blue-50 border-blue-200'} border`}>
                <p className={`text-sm ${isDark ? 'text-blue-300' : 'text-blue-700'}`}>
                  <strong>Ważne:</strong> Przedłużenie hostingu zostanie aktywowane po potwierdzeniu płatności. Może to zająć kilka minut.
                </p>
              </div>
            </div>
          </div>
