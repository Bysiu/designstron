...
export default function ZamowienieSzczegoly() {
  ...
  const proceedToPayment = async () => {
    ...
    setIsPaying(true);
    try {
      const response = await fetch('/api/stripe/create-checkout-session-for-order', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(data?.message || 'Wystąpił błąd podczas tworzenia sesji płatności');
      }

      const stripe = await stripePromise;
      if (!stripe) {
        throw new Error('Stripe nie jest dostępny');
      }
      ...
      console.error('Błąd podczas przekierowania do płatności:', error);
      alert(error.message || 'Wystąpił błąd podczas przekierowania do płatności');
    } finally {
      setIsPaying(false);
    }
  };
  ...
  return (
    <div data-theme={isDark ? 'dark' : 'light'} className={`min-h-screen ${bgClass} overflow-hidden transition-colors duration-500 relative`}>
      <Head>
        <title>Szczegóły zamówienia - Designstron</title>
      </Head>
      ...
      <NavbarAuth isDark={isDark} setIsDark={noopSetIsDark} currentPage="panel" />

      <main className="relative pt-32 px-4 sm:px-6 lg:px-8 pb-12">
        <div className="max-w-7xl mx-auto">
          <div className="mb-6">
            <Link href="/panel" className="text-blue-400 hover:text-blue-300 flex items-center">
              ...
              {order.status === 'PENDING' && (
                <div className="mb-6">
                  <button
                    ...
                    disabled={isPaying}
                    className="w-full sm:w-auto bg-gradient-to-r from-blue-600 to-purple-600 text-white px-6 py-3 rounded-lg font-semibold hover:from-blue-700 hover:to-purple-700 transition-all disabled:opacity-50"
                  >
                    {isPaying ? 'Przekierowanie...' : 'Przejdź do płatności'}
                  </button>
                </div>
              )}
          ...
          <div className="lg:col-span-1">
            <div className={`${cardBg} backdrop-blur-sm rounded-2xl border h-[600px] flex flex-col overflow-hidden`}>
              <div className={`${isDark ? 'border-slate-800' : 'border-gray-200'} p-4 border-b`}>
                ...
                <p className={`text-sm ${textSecondary}`}>Komunikacja z zespołem DesignStron.pl</p>
              </div>
                    ...
                    >
                      ...
                      >
                        ...
                        <p className={`text-xs mt-1 ${
                          message.sender === 'USER' ? 'text-blue-100' : isDark ? 'text-slate-400' : 'text-gray-500'
                        }`}
                        >
